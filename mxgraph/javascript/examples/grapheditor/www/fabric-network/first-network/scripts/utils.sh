...
packageChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode package ttcc.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label ttcc_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode install ttcc.tar.gz >&log.txt
  res=$?
  set +x
  cat log.txt
...
checkCommitReadiness() {
  ...
  do
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name ttcc $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --output json  >&log.txt
    res=$?
    set +x
...
queryCommitted() {
  ...
  do
    ...
    set -x
    peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name ttcc >&log.txt
    res=$?
    set +x
